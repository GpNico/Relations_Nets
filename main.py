...
def multi_sprite_experiment():
    conf = config.multi_sprite_config
                                    
    trainset = datasets.MultiObjectDataset(conf.data_dir, train=True, split = 0.9)
    
    #print("debug ", len(trainset))
    trainloader = datasets.MultiObjectDataLoader(trainset,
                                              batch_size=conf.batch_size,
                                              shuffle=True)
    monet = model.Monet(conf, 64, 64).cuda()
    if conf.parallel:
